...
final class PaymentController: UIViewController {
    ...
    private lazy var fixedButton: UIButton = {
        let button = UIButton()
        button.titleLabel?.textColor = .enBlue
    ...
    public func setupButtonColor() {
        if paymentTextField.text?.isEmpty == false {
            fixedButton.backgroundColor = .enBlue
            fixedButton.titleLabel?.textColor = .white
        }else {
            fixedButton.backgroundColor = .white
            fixedButton.titleLabel?.textColor = .enBlue
        }
    }
